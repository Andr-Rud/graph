...
from examples import run_inverted_index, run_pmi, run_yandex_maps, run_word_count


def test_run_word_count() -> None:
    file_input, file_output = "input.txt", "output.txt"
    ...
    CliRunner().invoke(run_word_count.main, file_input + " " + file_output)
    with open(file_output) as f:
        for i, line in enumerate(f):
            assert line.strip() == str(expected[i])
...
def test_inverted_index() -> None:
    ...
    CliRunner().invoke(run_inverted_index.main, file_input + " " + file_output)
    with open(file_output) as f:
        result = [json.loads(line.replace('\'', '\"')) for line in f]

    result.sort(key=itemgetter("doc_id", "text"))
    assert result == expected


def test_run_pmi() -> None:
    file_input, file_output = "input.txt", "output.txt"
    ...
    CliRunner().invoke(run_pmi.main, file_input + " " + file_output)
    with open(file_output) as f:
        result = [json.loads(line.replace('\'', '\"')) for line in f]
...
def test_run_yandex_maps() -> None:
    file_input_1, file_input_2, file_output = "input1.txt", "input2.txt", "output.txt"
    ...
    CliRunner().invoke(run_yandex_maps.main, file_input_2 + " " + file_input_1 + " " + file_output)
    with open(file_output) as f:
        result = [json.loads(line.replace('\'', '\"')) for line in f]
    assert result == expected
